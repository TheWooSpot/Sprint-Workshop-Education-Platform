...
function Navbar({ toggleSidebar }) {
  const { currentUser, userProfile, logout, bypassMode } = useAuth()
  ...
  const displayName = bypassMode 
    ? 'Workshop Guest' 
    : (currentUser?.displayName || 'User')

  // Determine profile initial for avatar
  const profileInitial = bypassMode 
    ? 'G' 
        ...
        <div className="relative">
          ...
          >
            <div className="w-10 h-10 rounded-full bg-primary flex items-center justify-center text-white">
              {profileInitial || <FaUser />}
            </div>
            <span className="hidden md:block font-medium truncate max-w-[120px]">
              {displayName}
            </span>
            {bypassMode && (
              <span className="hidden md:block text-xs bg-accent-green text-white px-2 py-0.5 rounded-full">
                Guest
              </span>
            )}
          </button>
          ...
          {profileOpen && (
            ...
            >
              {bypassMode && (
                <div className="px-4 py-2 text-sm text-accent-green border-b border-background-light">
                  Quick Access Mode
                </div>
              )}
              <Link 
              ...
              >
                {bypassMode ? 'Exit Guest Mode' : 'Logout'}
              </button>
            </motion.div>
          )}
        </div>
